...
def get_organisation_and_patch_locations_for_remapped_country_id():
    ...
    # Configure logging
    logging.basicConfig(filename='get_organisation_and_patch_location_for_remapped_country_id_case_3_1.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    def process_row(row):
        ...
        if isinstance(city_id, str) or isinstance(remapped_country_id, int) or pd.isnull(remapped_country_id) or remapped_country_id.strip() == "" or remapped_country_id in ("CORRECT", "CHECKED"):
            return result_dict
    ...
    def group_by_country_id(city_id_and_country_id_list):
        country_id_mapping = {}
        for city_id_and_country_id_dict in city_id_and_country_id_list:
            for city_id, country_id in city_id_and_country_id_dict.items():
                if country_id not in country_id_mapping:
                    country_id_mapping[country_id] = []
                country_id_mapping[country_id].append(city_id)
    ...
    def patch_locations(locations_to_patch):
        ...
        for location_id, payload in locations_to_patch:
            ...
            try:
                ...
                logging.error(f"Error occurred while patching location data for location {location_id}: {e}")
    ...
    def get_all_locations_to_patch():
        ...
        country_id_mapping = group_by_country_id(cities_id_and_remapped_country_id_list)
        for remapped_country_id, city_ids in country_id_mapping.items():
...
# print(f"Elapsed time - [get_organisation_and_patch_locations_for_remapped_country_id FUNCTION] : {elapsed_time:.2f} seconds")
